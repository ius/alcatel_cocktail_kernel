...
#if defined(CONFIG_MPU_SENSORS_MPU3050) && defined(CONFIG_MPU_SENSORS_BMA250)	
extern bool Gyroexist(void);
#endif
...
/*************fanxin add2011.10.25*****************************************/
/*struct bma250acc bma250_get_acc(struct bma250_data *bma250_datap)
{
    unsigned short int lsb,msb;
    struct bma250acc acc;

    lsb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Z_AXIS_LSB_REG);
    msb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Z_AXIS_MSB_REG);
    printk(KERN_ALERT "####Show the lsb_z's value: %d\n,show the msb_z's value: %d\n",lsb,msb);
    lsb = lsb >> 6;
    msb = msb << 2;
    acc.z = msb | lsb;
    
    lsb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Y_AXIS_LSB_REG);
    msb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Y_AXIS_MSB_REG);
    printk(KERN_ALERT "####Show the lsb_y's value: %d\n,show the msb_y's value: %d\n",lsb,msb);
    lsb = lsb >> 6;
    msb = msb << 2;
    acc.y =msb | lsb;
    ...
    lsb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Z_AXIS_LSB_REG);
    msb = i2c_smbus_read_byte_data(bma250_datap->bma250_client, BMA250_Z_AXIS_MSB_REG);
    printk(KERN_ALERT "####Show the lsb_x's value: %d\n,show the msb_x's value: %d\n",lsb,msb);
    lsb = lsb >> 6;
    msb = msb << 2;
    acc.x = msb | lsb;
...
//	acc = bma250_get_acc(bma250);
/*	printk(KERN_ALERT "#####bma250_work_func is running!,so acc.x = %u, acc.y = %u, acc.z = %u#####\n",acc.x, acc.y, acc.z);
	
	printk(KERN_ALERT "####This is the bma250's bma250_report_fuc()!####\n");
        for(i = 0; i<BITS_TO_LONGS(EV_CNT); i++)
        {
	    printk(KERN_ALERT "####So We will show the evbit's neirong: %lu####\n",bma250->input->evbit[i]);
        }*/
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	if ((data == 0)||(data==1)) {
//		printk(KERN_INFO "####The bma250_enable_store()function's data is %ld####\n",data);
		bma250_set_enable(dev,data);
	}

	return count;
}
//PR-275586-yanqi.liu-begin
...
/*	printk(KERN_ALERT "####This is the bma250's input register!####\n");
	for(i = 0; i<BITS_TO_LONGS(EV_CNT); i++)
    	{
		printk(KERN_ALERT "####So We will show the evbit's neirong: %lu####\n",dev->evbit[i]);
    	}*/
...
/*	printk(KERN_ALERT "####This is showed after input_set_capability(),let me see the change!####\n");
    	for(i = 0; i<BITS_TO_LONGS(EV_CNT); i++)
    	{
		printk(KERN_ALERT "####So We will show the evbit's neirong: %lu####\n",dev->evbit[i]);
    	}*/
		...
		const struct i2c_device_id *id)
{
	...
	printk(KERN_ALERT "#####BMA250 IS runing now!#####\n");
	...
	/* read chip id */
	tempvalue = 0;
	printk(KERN_ALERT "####Bma250 is running here!,the addr is 0x%x\n",client->addr);
	...
	if(Gyroexist() == true)
	{
		printk(KERN_INFO "####The Gyroexist() == true is true, so the code is returned!####\n");
		return err;
	}
#endif
	...
	if (err < 0)
	{
//		printk(KERN_ALERT "####The sysfs_create_group() is returned the err, so The code is returned!####\n");
		goto error_sysfs;
	}
...
//	printk(KERN_ALERT "#####BMA250 IS runing now!,The driver's probe will over!#####\n");
//	bma250_set_enable(&data->bma250_client->dev,1);
	return 0;
