...
dhd_pktfilter_offload_enable(dhd_pub_t * dhd, char *arg, int enable, int master_mode)
{
	...
	rc = rc >= 0 ? 0 : rc;
	if (rc)
		DHD_ERROR(("%s: failed to add pktfilter %s, retcode = %d master_mode = %d\n",
		__FUNCTION__, arg, rc, master_mode));
	else
...
int dhd_keep_alive_onoff(dhd_pub_t *dhd, int ka_on)
{
	...
	if (ka_on) { /* on suspend */
		mkeep_alive_pkt.period_msec = 15000;
	...
	null_data = NULL_PKT_STR; 
	mkeep_alive_pkt.len_bytes = 0 ;//strlen(null_data);
	...
	memcpy(&buf[buf_len], null_data, mkeep_alive_pkt.len_bytes);	
	buf_len += mkeep_alive_pkt.len_bytes;
	printk("dhd buf_len %d \n", buf_len);	
	
	printk("dhd %d, %d \n", sizeof(mkeep_alive_pkt), WL_MKEEP_ALIVE_FIXED_LEN);

	res = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, buf_len, TRUE, 0);
	
	printk("dhd shaohua test res %d \n", res );
