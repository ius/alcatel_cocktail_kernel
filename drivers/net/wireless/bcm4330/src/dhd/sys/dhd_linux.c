...
#if defined(FILTER_KEEPALIVE_PKT)
uint dhd_master_mode = FALSE;
#else
uint dhd_master_mode = TRUE;
#endif  //#if defined(FILTER_KEEPALIVE_PKT)
...
//harry
int net_os_set_power_mode(struct net_device *dev, int val)

{
    dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
    dhd_wl_ioctl_cmd(&dhd->pub, WLC_SET_PM, (char *)&val,  sizeof(val), TRUE, 0);
...
void
dhd_btcoext_set_pkt(int value, struct net_device *dev)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
	dhd_set_packet_filter(value,&dhd->pub);
}
...
static int dhd_set_pfn(dhd_pub_t *dhd, int enabled)
{
	 ...
	 DHD_PFN((" %s enable pfn: %d, ret = %d\n", __func__,  pfn_enabled, ret));
	...
	if (iov_len) {
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, 0)) < 0) {
			DHD_ERROR(("%s failed pfnclear %d\n", __FUNCTION__, ret));
		}
	}

	DHD_PFN((" %s clear pfn  ret = %d \n", __func__, ret)); 
	...
	memset(&pfn_element, 0, sizeof(pfn_element));
	 ...
	 memset(&iovbuf, 0, sizeof(iovbuf));
        iov_len = bcm_mkiovar("pfn_set", (char *)&pfn_param, sizeof(pfn_param), iovbuf, sizeof(iovbuf));
        ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, 0);
	 DHD_PFN((" %s set pfn: %s ret = %d\n", __func__,  pfn_ssid_set.ssid, ret));
	 ...
	 pfn_element.infra = htod32(1);
        ...
        memcpy((char *)pfn_element.ssid.SSID, pfn_ssid_set.ssid, sizeof(pfn_element.ssid.SSID));
        pfn_element.ssid.SSID_len = pfn_ssid_set.ssid_len;

        iov_len = bcm_mkiovar("pfn_add", (char *)&pfn_element, sizeof(pfn_element), iovbuf, sizeof(iovbuf));
        ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, 0);
        DHD_PFN((" %s add pfn: %s ret = %d\n", __func__, pfn_element.ssid.SSID, ret));
...
#endif   //WLAN_PFN
extern void wl_iw_add_enable_pfn(int n);
#if defined(CONFIG_HAS_EARLYSUSPEND)
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
				...
				if((false == g_disableDhcpPktFilter) || !ap_fw_loaded)
				dhd_set_packet_filter(1, dhd);

#ifdef WLAN_PFN
                /* set pfn */
		 		printk("%s call :dhd_enable_pfn_ssid \n", __func__);
                wl_iw_add_enable_pfn(1);
#endif
...
int net_os_send_hang_message(struct net_device *dev)
{
	...
	//		ret = wl_cfg80211_hang(dev, WLAN_REASON_UNSPECIFIED);
			bcm_mdelay(500); /* waiting for FW reload */
#endif
		}
	}
	return ret;
