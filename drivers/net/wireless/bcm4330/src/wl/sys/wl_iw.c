...
extern void dhd_btcoext_set_pkt(int value, struct net_device *dev);
...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		{					        
			if(1 == g_bEarlySuspendMode && !ap_fw_loaded)
			{		
			        WL_ERROR(("Enable filter +\n"));
							//dhd_set_packet_filter(1, pfn_dhd);		
				 			dhd_btcoext_set_pkt(1, dev);			
			        WL_ERROR(("Enable filter -\n"));				
			}			
...
#include <linux/wakelock.h>
static struct wake_lock wl_pfn_wake;
struct net_device *pfn_dev;

void wl_iw_add_enable_pfn(int n)
{
	...
	if(n){
		if ((dev_wlc_ioctl(pfn_dev, WLC_GET_SSID, &pfn_ssid, sizeof(pfn_ssid)))) {
			WL_ERROR(("%s Error getting the SSID\n", __func__));
			return ;
		} else {
			printk("shaohua success get ssid %s \n", pfn_ssid.SSID);
		}

		 if(pfn_ssid.SSID_len > 0) {
		 	printk("shaohua set and enable ssid into pfn\n");
	        	dhd_set_pfn_ssid(pfn_ssid.SSID, pfn_ssid.SSID_len);
...
char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	char mac_lst[384];
	char *p_mac_str=NULL;
	char *p_mac_str_end=NULL;
	 printk("stalist_read_proc+++++\n");
	 if(off==0)
		 {
		 		memset(sta_maclist, 0, sizeof(mac_buf));

				sta_maclist->count = 8;
			 if ((ret = get_assoc_sta_list(ap_net_dev, mac_buf, sizeof(mac_buf))) < 0) 
				printk("%s: get_assoc_sta_list sta list ioctl error:%d\n",	__FUNCTION__, ret);		

			 printk("+++++ get_assoc_sta_list 0x%x %d\n",(unsigned int )ap_net_dev,sizeof(mac_buf));
			 WL_SOFTAP(("%s: got %d stations\n", __FUNCTION__,
				 sta_maclist->count));
			 ...
			 if(sta_maclist->count>8)
			 {
			 	printk("sta_maclist->count>8!\n");
				return 0;
			 }			 
				 for ( i = 0; i < sta_maclist->count; i++) { 
					 ...
					 if (!ETHER_ISNULLADDR(id->octet)) {
						 ...
						 if ((p_mac_str_end - p_mac_str) <= 36) {
			 			 ...
			 			 //if(i>0)
						 //	p_mac_str += snprintf(p_mac_str, 1, "\n");
			 ...
			 //get_assoc_sta_list(priv_dev,tempbuf,4096);
			 printk("get_assoc_sta_list 0x%x,%s\n",(unsigned int)ap_net_dev,mac_lst);
		 }	 
		 len=strlen(mac_lst);		 
	return common_read_proc(page,start,off,count,eof,data,mac_lst,len);	
...
wl_iw_attach(struct net_device *dev, void * dhdp)
{
	...
	d_entry = create_proc_entry("WIFI_STA_LIST",0, NULL);
	if (d_entry)
	{
		d_entry->read_proc = stalist_read_proc;
...
#endif
	remove_proc_entry("WIFI_STA_LIST",NULL);
