...
extern int wl_android_set_bt_wifi_coexist(struct net_device *dev , int val);
#ifdef WL_CFG80211
...
int wl_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		bytes_written = wl_cfg80211_set_btcoex_dhcp(net, command);
                    ...
                    uint mode = *(command + strlen(CMD_POWERMODE) + 1) - '0';
 
       if (mode == 1) {
           net_os_set_power_mode(net, 0); /* DHCP starts */
		   wl_android_set_bt_wifi_coexist(net, 0);
	   }
       else {
           net_os_set_power_mode(net, 2); /* DHCP ends */
		   wl_android_set_bt_wifi_coexist(net, 1);
	   }
    }
...
#if 1 //penguin, temporary remove, the iface name should be determin when insert module
//#ifdef CUSTOMER_HW2
	if (!iface_name[0]) {
