    ...
    defined(CONFIG_MPU_SENSORS_MPU6000_MODULE)
		...
		if (pdata->accel.get_slave_descr) {
			...
			if(mldl_cfg->accel->init){
				if(mldl_cfg->accel->init(accel_adapter, mldl_cfg->accel, &pdata->accel) != ML_SUCCESS){
					mldl_cfg->accel = NULL;
					accel_adapter = NULL;
					goto configure_ecompass;
				}
			}
...
configure_ecompass:
		...
		if (pdata->compass.get_slave_descr) {
			...
			if(mldl_cfg->compass->init){
				if(mldl_cfg->compass->init(compass_adapter, mldl_cfg->compass, &pdata->compass) != ML_SUCCESS){
					mldl_cfg->compass = NULL;
					compass_adapter = NULL;
					goto configure_pressure;
				}
			}
